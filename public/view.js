...
async function load(P, panoId, heading) {
  ...
  if (['apple','baidu','yandex','kakao'].includes(S.src) && Number.isFinite(meta.yaw))
...
function updateAttr() {
  ...
  const srcN = S.src === 'apple' ? (EN_UI ? 'Apple Look Around' : 'Apple Look Around')
             ...
             : S.src === 'yandex' ? 'Yandex'
             : S.src === 'kakao' ? (EN_UI ? 'Kakao' : 'Kakao 街景')
...
window.__turn = (cmd, text) => {
  ...
  if (cmd === 'aiabout') {                          // 「介紹○○」= 指名介紹
    ...
    const subject = (me ? me[2] : mz ? mz[1] : '').replace(/^一下/, '');
...
addEventListener('keydown', e => {
  ...
  // 只改「顯示朝向」,不影響行進方向(道路照跑)。改了要重載目前這顆才套用新 yaw。
    ...
    localStorage.setItem('pano-aflip6', S.aflip);
...
(async () => {
  ...
  S.aflip = localStorage.getItem('pano-aflip6') != null ? +localStorage.getItem('pano-aflip6') : 0;   // 整體前後(預設0=正前,實測);\\ 鍵切換記住
