...
(function () {
  ...
  const parse = t => {
    ...
    // 「介紹○○」指名介紹(名字 2~20 字;之前誤解使用者的 bug 回報
    // 而停用過 —— 使用者要的一直是「介紹景點名」要能整句聽懂)。
    // 搶跑防護:settle 分派會等整句穩定;超過 20 字的閒聊長句
    // 由 cancelIfLonger 收回排隊,不會誤觸。
    const mAb = x.match(/^介紹(.{2,22})$/);
    if (mAb) {
      const n = mAb[1].replace(/^一下/, '');
      // 帶疑問/請求語感的是聊天句(問 Gemini 的),不是指名景點
      if (n.length >= 2 && !/什麼|甚麼|嗎|怎麼|如何|哪|請|幫|有沒有|推薦|好吃|好玩/.test(n)) return 'aiabout';
      return null;
    }
    return null;
  };
