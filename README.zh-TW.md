...
**老實說清楚。** 全景影像是**免金鑰**直接打各家內部圖磚端點抓的——Google 街景、Apple Look Around、百度全景(中國大陸)、Yandex(俄羅斯/中亞/土耳其)、Kakao(韓國)。這些**都不在任何一家的授權範圍內**。每一家都有官方、要金鑰、要付費的 API,他們的服務條款都要求你走官方 API。**繞過 API 直接抓圖磚,違反每一家的服務條款。** 這是灰色地帶,本專案只以個人研究的定位存在。

白話講:

- **法律性質**:違反服務條款主要是**民事違約**(違反使用契約),不是犯罪。影像的著作權屬於各家業者。
- **個人、私下、不散布的用途**(自己在跑步機上看、即時算圖、沒有大量存檔)——實務上風險很低,但仍然違反條款。
- **紅線,別跨:**
  - ❌ **不要公開部署**給別人用。
  - ❌ **不要散布影像**(少量截圖討論是一回事;大量下載、重新架站就不行)。
  - ❌ **不要商業化**、不要做成競爭服務。
- **要完全合規**,就把影像層改用各家官方付費 API(金鑰、綁帳單),或改接**開放授權影像**——[Mapillary](https://www.mapillary.com) 或 KartaView(CC 授權、群眾上傳;覆蓋較稀、360°/平面畫質參差)。

其它資料:

- **景點資料**來自 Wikidata/維基百科(CC BY-SA,介面有標示出處),照片來自 Wikimedia Commons,地圖磚來自 OpenStreetMap。
- **海拔**來自 Open-Meteo(免費、免金鑰)。**TTS 與 Gemini** 用你自己的金鑰、自己的額度。

使用本程式碼的方式由你自行負責。自己玩就好,別跨上面那幾條紅線。
